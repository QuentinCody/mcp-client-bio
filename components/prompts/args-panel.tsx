"use client";
import { X, AlertCircle, CheckCircle2, HelpCircle, Loader2, Sparkles } from "lucide-react";
...
export function ArgsPanel({
  ...
  onClose: () => void;
  onCompleteArgument?: (argName: string, value: string, context: Record<string, string>) => Promise<string[]>;
}) {
  const [focusedField, setFocusedField] = useState<string | null>(null);
  const completionTimersRef = useRef<Record<string, number>>({});
  const requestCountersRef = useRef<Record<string, number>>({});
  const [suggestions, setSuggestions] = useState<Record<string, { values: string[]; loading: boolean }>>({});
  ...
  useEffect(() => {
    setSuggestions({});
    requestCountersRef.current = {};
    for (const timer of Object.values(completionTimersRef.current)) {
      clearTimeout(timer);
    }
    completionTimersRef.current = {};
    if (onCompleteArgument) {
      const snapshot = { ...values };
      for (const arg of args) {
        scheduleCompletion(arg.name, snapshot);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [args, onCompleteArgument]);

  const scheduleCompletion = (argName: string, nextValues: Record<string, string>) => {
    if (!onCompleteArgument) return;
    if (completionTimersRef.current[argName]) {
      clearTimeout(completionTimersRef.current[argName]);
    }
    const nextCounter = (requestCountersRef.current[argName] ?? 0) + 1;
    requestCountersRef.current[argName] = nextCounter;
    completionTimersRef.current[argName] = window.setTimeout(async () => {
      try {
        setSuggestions((prev) => ({
          ...prev,
          [argName]: {
            values: prev[argName]?.values ?? [],
            loading: true,
          },
        }));
        const result = await onCompleteArgument(argName, nextValues[argName] ?? '', nextValues);
        if (requestCountersRef.current[argName] !== nextCounter) return;
        const unique = Array.isArray(result)
          ? Array.from(new Set(result.filter((entry): entry is string => typeof entry === 'string' && entry.trim().length > 0)))
          : [];
        setSuggestions((prev) => ({
          ...prev,
          [argName]: {
        ...
        console.warn('[ArgsPanel] completion fetch failed for', argName, error);
        if (requestCountersRef.current[argName] !== nextCounter) return;
        setSuggestions((prev) => ({
          ...prev,
          [argName]: {
  ...
  const handleSuggestionSelect = (argName: string, suggestion: string) => {
    const nextValues = { ...values, [argName]: suggestion };
    onChange(argName, suggestion);
    scheduleCompletion(argName, nextValues);
  };
            ...
            <div key={a.name} className="space-y-1.5">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  const nextValues = { ...values, [a.name]: nextValue };
                  onChange(a.name, nextValue);
                  scheduleCompletion(a.name, nextValues);
                }}
                onFocus={() => {
                  setFocusedField(a.name);
                  scheduleCompletion(a.name, values);
                }}
              ...
              {onCompleteArgument && (
                <div className="mt-1 min-h-[1.25rem]">
                  {suggestions[a.name]?.loading ? (
                    <div className="flex items-center gap-1 text-[11px] text-amber-700">
                      <Loader2 className="w-3 h-3 animate-spin" />
                      <span>Fetching suggestions…</span>
                    </div>
                  ) : suggestions[a.name]?.values?.length ? (
                    <div className="flex flex-wrap gap-1.5">
                      {suggestions[a.name].values.slice(0, 6).map((option, idx) => (
                        <button
                          key={`${option}-${idx}`}
                          type="button"
                          onClick={() => handleSuggestionSelect(a.name, option)}
                          className="px-2 py-0.5 text-[11px] rounded-full border border-amber-200 bg-white text-amber-700 hover:bg-amber-50 transition-colors"
                        >
                          {option}
                        </button>
                      ))}
