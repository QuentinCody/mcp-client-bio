...
export function generateTransformingHelpersImplementation(
  ...
  const lines: string[] = [
    ...
    '}',
    '',
    '// Task #4: Safe array extraction utility',
    '// Safely extracts arrays from API responses with auto-detection of common patterns',
    'function extractArray(result, path, fallback = []) {',
    '  // If result is already an array, return it',
    '  if (Array.isArray(result)) return result;',
    '  ',
    '  // If result is null/undefined, return fallback',
    '  if (result == null) return fallback;',
    '  ',
    '  // If a specific path is provided, try to extract from it',
    '  if (path) {',
    '    const value = safeGet(result, path, null);',
    '    return Array.isArray(value) ? value : fallback;',
    '  }',
    '  ',
    '  // Auto-detect common array locations in order of priority',
    '  const commonPaths = [',
    '    "data",           // GraphQL, REST common',
    '    "results",        // Search results',
    '    "items",          // Pagination common',
    '    "records",        // Database-style',
    '    "entries",        // Feed-style',
    '    "hits",           // Search engines (Elasticsearch)',
    '    "rows",           // Tabular data',
    '    "content",        // MCP responses',
    '    "structuredContent.data",    // MCP structured',
    '    "structuredContent.results", // MCP structured',
    '    "structuredContent.items",   // MCP structured',
    '    "response.data",  // Nested response',
    '    "body.data",      // HTTP response',
    '  ];',
    '  ',
    '  for (const p of commonPaths) {',
    '    const value = safeGet(result, p, null);',
    '    if (Array.isArray(value)) return value;',
    '  }',
    '  ',
    '  // Check if result has any array property',
    ...
    '}',
    '',
    '// Convenience wrapper that also handles .map() safely',
    'function safeMap(result, path, mapFn) {',
    '  const arr = extractArray(result, path);',
    '  if (typeof mapFn !== "function") return arr;',
    '  return arr.map(mapFn);',
    '}',
    '',
    '// Extract first item safely',
    'function extractFirst(result, path, fallback = null) {',
    '  const arr = extractArray(result, path);',
  ...
  lines.push('  // Task #4: Safe array extraction utilities');
  lines.push('  extractArray,');
  lines.push('  safeMap,');
  lines.push('  extractFirst');
  lines.push('};');
  lines.push('');
